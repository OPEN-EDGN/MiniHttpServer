...
class ByteArrayDataReader(private val data: ByteArray) : BaseDataReader {
 ...
 *  注意，这为临时文件方案，文件会自动删除
 */
class FileDataReader(private val file: File) : BaseDataReader {
    init {
        if (file.isFile.not()) {
...
fun String.createDataReader() = ByteArrayDataReader(this.toByteArray(Charsets.UTF_8))
