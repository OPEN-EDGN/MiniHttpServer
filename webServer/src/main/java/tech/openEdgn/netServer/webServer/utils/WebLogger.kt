...
class WebLogger constructor(private val clazz: Class<out Any>) {
    ...
    fun info(message: String, exception: Throwable? = null): WebLogger {
        outputLogger(Thread.currentThread().name, WebLoggerLevel.INFO, message, exception)
        return this
    }

    @JvmOverloads
    fun debug(message: String, exception: Throwable? = null): WebLogger {
        outputLogger(Thread.currentThread().name, WebLoggerLevel.DEBUG, message, exception)
        return this
    }

    @JvmOverloads
    fun warn(message: String, exception: Throwable? = null): WebLogger {
        outputLogger(Thread.currentThread().name, WebLoggerLevel.WARN, message, exception)
        return this
    }

    @JvmOverloads
    fun error(message: String, exception: Throwable? = null): WebLogger {
        outputLogger(Thread.currentThread().name, WebLoggerLevel.ERROR, message, exception)
        return this
    }
